...
const Portfolio = () => {
	...
	return (
		<section
			...
			<h1 className="text-2xl font-bold tracking-tight text-zinc-800 sm:text-2xl dark:text-zinc-100 px-10">
			...
			<div className=" grid grid-cols-1 md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-2 gap-2 sm:gap-1">
				{portfolioData.map((item) => (
					<div
						key={item.id}
						className="border mx-10 px-10 group group-hover:scale-100 group-hover:opacity-100 my-10 flex flex-col justify-start rounded">
						<div className="py-4 relative flex flex-col items-start">
							...
							<p className="relative z-10 mt-6 flex text-sm font-medium text-zinc-400 transition group-hover:text-teal-500 dark:text-zinc-200">
								...
								<span className="ml-2">{item.websiteUrl}</span>
							</p>
						</div>
					</div>
				))}
